...
if __name__ == '__main__':
    ...
    # prec = np.sum(q, axis=1)
    cwv = np.trapz(q, p/100, axis=1) / 9.8
    ...
    data_hov = np.average(cwv_eq, axis=1, weights=weights)
    time = np.arange(data_hov.shape[0])

    fig, ax = plt.subplots(figsize=(4, 8))
    ax.pcolormesh(x, time/4, data_hov, cmap='YlGnBu')

    ax.set_xlim(0, 360)
    ax.set_ylim(0, 200)
    ax.set_xlabel('Longitude')
    ax.set_ylabel('Day')    
    plt.savefig(f'/home/garywu/summer_2025/dycore/figures/hovmoller_prec.png', dpi=300, bbox_inches='tight')